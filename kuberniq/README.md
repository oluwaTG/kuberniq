...
A CLI for registering and managing remote Kubernetes clusters with the [Kuberniq MCP server](../mcp-server/README.md).

One command sets up everything in the target cluster and registers it with the MCP server so every endpoint gains `?cluster=<name>` routing.
